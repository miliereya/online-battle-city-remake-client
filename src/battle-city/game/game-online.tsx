...
import { CreateGameObject, TypeMoveButton } from '../types'
import { Players } from '../players/players'
import { Bullets } from '../bullets/bullets'
import { useFrameLoop, useMobile } from '../hooks'
...
export const GameOnline = () => {
	...
	useEffect(() => {
		...
		const fireMobileHandler1 = () => {
			if (game) lobbyActions.input('FIRE', game.id)
		}

		const pauseMobileHandler = () => {
			if (game) lobbyActions.input('SPACE', game.id)
		}
		...
		const moveTopHandler1 = () => {
			move1Timer = setInterval(() => {
				if (game) lobbyActions.input('TOP', game.id)
			}, 10)
		}

		const moveRightHandler1 = () => {
			move1Timer = setInterval(() => {
				if (game) lobbyActions.input('RIGHT', game.id)
			}, 10)
		}

		const moveLeftHandler1 = () => {
			move1Timer = setInterval(() => {
				if (game) lobbyActions.input('LEFT', game.id)
			}, 10)
		}

		const moveBottomHandler1 = () => {
			move1Timer = setInterval(() => {
				if (game) lobbyActions.input('BOTTOM', game.id)
			}, 10)
		}
